...
void keyboard (unsigned char key, int x, int y)
{
	...
	case 'R':
		...
		rotaEixoY = 0;
		rotaEixoZ = 0;
		break; //Volta TODO o Braço para a posição original
...
void display(void)
{
	...
	glRotatef ((GLfloat) rotaEixoX, 1.0, 0.0, 0.0);//rotação de todo o braço em torno do eixo X
	glRotatef ((GLfloat) rotaEixoZ, 0.0, 0.0, 1.0);//rotação de todo o braço em torno do eixo Z
	...
	// Restaura a largura das linhas para o valor padrão (1.0)
	glLineWidth(1.0);
	...
	glPopMatrix();
	glRotatef ((GLfloat) rotaEixoY, 0.0, 1.0, 0.0);//rotação de todo o braço em torno do eixo Y
	//Movimento do ombro
	glPushMatrix();
